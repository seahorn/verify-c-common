/*
 * untility function implementations that are purposefully
 * rigged to display incorrect behaviour for counter-examples
 */
...
size_t sea_strlen(const char *str, size_t max)
{
    ...
    /* removing all memory access for now */
    // assume(str[i] == '\0');
